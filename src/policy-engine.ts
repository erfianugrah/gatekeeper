...
// ─── v1 scope migration ────────────────────────────────────────────────────
// Convert old-style scopes to a policy document.
...
/** Convert v1 scopes to a v2 policy document. */
export function migrateV1Scopes(scopes: V1Scope[], zoneId: string): PolicyDocument {
	// Wildcard scope = allow everything on this zone
	...
	// No scopes = unrestricted on this zone (backward compat)
	if (scopes.length === 0) {
		return {
			version: POLICY_VERSION,
			statements: [
	...
	// Group by scope type to create statements
	const statements: Statement[] = [];

	for (const scope of scopes) {
		const stmt = migrateOneScope(scope, zoneId);
		if (stmt) statements.push(stmt);
	}

	// If no statements were created (shouldn't happen), default to deny-all
...
function migrateOneScope(scope: V1Scope, zoneId: string): Statement | null {
	...
	switch (scope.scope_type) {
		case 'host':
			return {
				...
				actions: ['purge:*'],
				resources: [resource],
				conditions: [{ field: 'host', operator: 'eq', value: scope.scope_value }],
			};
		case 'tag':
			return {
				effect: 'allow',
				actions: ['purge:*'],
				resources: [resource],
				conditions: [{ field: 'tag', operator: 'eq', value: scope.scope_value }],
			};
		case 'prefix':
			return {
				effect: 'allow',
				actions: ['purge:*'],
				resources: [resource],
				conditions: [{ field: 'prefix', operator: 'starts_with', value: scope.scope_value }],
			};
		case 'url_prefix':
			return {
				effect: 'allow',
				actions: ['purge:url'],
				resources: [resource],
				conditions: [{ field: 'url', operator: 'starts_with', value: scope.scope_value }],
			};
