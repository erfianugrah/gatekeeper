...
| `GATEKEEPER_URL=http://localhost:8787 bun run smoke`     | Run the full multi-surface live-API smoke against a deployment |
| `GATEKEEPER_URL=… GATEKEEPER_ADMIN_KEY=… bun run smoke:supabase` | Supabase-only smoke (API-first synthetic + opt-in live PAT tier; optional metrics/write probes; no CF creds; the CI `live-smoke` entry) |
| `bun run env:decrypt`                                    | Decrypt `.env.sops` → `.env` (needs the SOPS age key); `env:edit` to change a value |
...
`bun run smoke` (entry `cli/smoke-test.ts`) exercises a deployment end-to-end over HTTP. `BASE = GATEKEEPER_URL ?? http://localhost:8787`; `IS_REMOTE` when `https://`. Each surface is a module under `cli/smoke/` (admin, purge, s3, dns, cf-proxy, **supabase**, …). The **supabase** module is API-first: a synthetic tier (always runs, no real credential — asset-layer wiring, auth ordering, classifier + deny-by-default, token binding) plus an opt-in live PAT tier gated on `SUPABASE_SMOKE_PAT` that registers a real PAT and exercises the **real Supabase Management API** through the proxy with a Gatekeeper key as Bearer (auth → classify → policy → PAT swap → real Supabase). Optional live checks: `SUPABASE_SMOKE_ENABLE_WRITE_PROBE=1` for a non-destructive write-classified probe and `SUPABASE_SMOKE_METRICS_SECRET` + `SUPABASE_SMOKE_METRICS_REF` for `/supabase/metrics/:ref` (experimental/optional). Required scope validation is the upstream API path through Gatekeeper — not CLI behavior. The official `supabase` CLI *can* be driven through the proxy as an additional compatibility check: a key bound to a `supabase` (PAT) upstream token is auto-minted in the CLI-compatible shape `sbp_` + 40 hex (matching the CLI's client-side regex `^sbp_(oauth_)?[a-f0-9]{40}$`), and the gateway looks keys up verbatim without enforcing a prefix. See `docs/GUIDE.md` §2.4 "Driving the official `supabase` CLI through the proxy" for the profile-file + env recipe. The shape decision lives in `IamManager.generateKeyId` (`src/iam.ts`), keyed off the bound upstream token's `scope_type`; rotation preserves it. The full `bun run smoke` is NOT in CI preflight (it needs the complete upstream credential set and creates real resources across all surfaces) — run it manually / on a schedule. The Supabase surface alone IS exercised in CI by the `live-smoke` job via the focused `cli/smoke-supabase.ts` entry (`bun run smoke:supabase`), which needs no CF creds and runs post-staging-deploy.

#### Supabase scope testing methodology (required)

- Test scope behavior through **raw proxy API paths first** (`/supabase/v1/*`, supported `/supabase/v0/*`), not CLI wrappers.
- For each action category, prove both sides:
  - **allow path** reaches upstream (not `401`/`403`; status may be `2xx/4xx/5xx` from Supabase itself), and
  - **deny path** returns `403` when action/resource is out of policy.
- Use a real upstream PAT (`SUPABASE_SMOKE_PAT`) for live tier assertions so policy checks reflect what upstream access actually allows.
- Keep the write-classified check non-destructive (`POST /database/query` with `select 1`) and gated behind `SUPABASE_SMOKE_ENABLE_WRITE_PROBE=1`.
- Treat `/supabase/metrics/:ref` live success checks as optional until metrics-secret provisioning is broadly available.
- Official `supabase` CLI checks are compatibility-only; they never replace API-level scope assertions.
...
- **CD lives in `ci.yml`** (`deploy` job, `needs: [preflight, e2e]` so a red suite or failed e2e never ships): push to `main` → staging, push tag `v*` → production, `workflow_dispatch` → chosen env. Auth is the scoped `CLOUDFLARE_API_TOKEN` repo secret (Workers Scripts + D1 on the account, Workers Routes + Zone Read on erfi.io) — **never** the global API key. A `live-smoke` job (`needs: deploy`, push-to-`main` only) then runs `cli/smoke-supabase.ts` against the freshly deployed staging worker — synthetic tier always, real PAT-swap live tier when `SUPABASE_SMOKE_PAT` is set; targets staging via `STAGING_ADMIN_KEY` and self-skips when unset. Gated to push-to-`main` (never `workflow_dispatch`, which can target prod) because it mints real resources via the admin key.
...
- `src/supabase/router.ts` — two proxy surfaces, both authorize BEFORE resolving the upstream credential (so unauthenticated callers can't probe which refs have a stored credential via 502-vs-401). **Both resolve the credential by the key's bound `upstream_token_id` (`auth.upstreamTokenId`), NOT by a scope/ref match** — `resolveUpstreamTokenById` for the PAT path, `resolveSupabaseMetricsCredentialById` for metrics; the scope-based `resolveSupabaseToken` / `resolveSupabaseMetricsCredential` are only a fallback for legacy unbound keys. This mirrors the CF/S3/purge proxies and guarantees a key can only ever use the exact credential it was bound to (a scope/ref resolution would pick the newest matching token, letting one key get another's credential and forwarding a bogus wildcard token on account-level calls — the `JWT could not be decoded` class of bug):
  - `ALL /supabase/v1/*` and `ALL /supabase/v0/*` — Management API, swaps in the bound PAT (Bearer). The `/v0` surface is treated as external/unstable: only `GET /v0/projects/{ref}/analytics/metrics` is classified, everything else under `/v0` denies by default.
  - Both surfaces consume a shared account-level token bucket (`stub.consumeSupabaseRateLimit()`, config `supabase_rps` / `supabase_burst`, default 200/400) AFTER authorization, BEFORE credential resolution — mirrors the S3 (`consumeS3RateLimit`) and CF (`consumeCfProxyRateLimit`) proxies. Exhaustion → `429` + `Retry-After`, breadcrumb `supabase-mgmt-rate-limited` / `supabase-metrics-rate-limited`.
  - `GET /supabase/metrics/:ref` — per-project metrics over HTTP Basic, swapping in the bound secret-key credential, streaming Prometheus text through unchanged.
- **Binding validation** (`src/routes/token-binding.ts`, `validateSupabaseResources`): a key bound to a `supabase` token may carry `project:<ref>` / `org:` / `branch:` / `supabase:account` resources. `project:<ref>` must be covered by the token's `zone_ids` unless wildcard; **`supabase:account` and `project:*` (both account-wide reach) require a wildcard token (`zone_ids: ["*"]`)** — so a ref-scoped token can't mint a key that enumerates the whole account. Because request-time resolution trusts the binding unconditionally, this account-wide guard lives at bind time, not as a per-request check.
- **Three metrics backends, same action** (`supabase:metrics:read`): the Basic-auth secret-swap path (`/supabase/metrics/:ref`), the experimental PAT path (`/supabase/v0/projects/{ref}/analytics/metrics`), and the stable v1 PAT path (`/supabase/v1/projects/{ref}/analytics/endpoints/metrics`, longest-prefix-matched to `metrics` before the generic `analytics` category). Pick whichever credential is on file; none makes assumptions about upstream internals.
...
- `src/supabase/membership-analytics.ts` - `supabase_membership_events` D1 table (plan Task 5 audit trail for the invitations surface) + `logMembershipEvents` (fire-and-forget, batch) + `queryMembershipEvents` / `queryMembershipSummary` / `deleteOldMembershipEvents`. One row per planned change (`preview`/`denied`), per `noop`, and per blocked assignment (`blocked` - member writes are not PAT-drivable). Wired in `src/supabase/member-router.ts` (dry-run + 501 branches); admin reads at `GET /admin/supabase/membership/{events,summary,timeseries}` (`src/routes/admin-supabase-membership.ts`). NOTE: the membership table has no `status` column - its timeseries passes `errorCondition: "outcome IN ('denied','failed')"` to `queryTimeseries` (the default `status >= 400` only fits request-analytics tables).
- CLI: `gk supabase-analytics {events,summary,timeseries}` (`cli/commands/supabase-analytics.ts`), registered in `cli/index.ts` + `cli/commands/completions.ts`. Membership audit has the same shape: `gk supabase-membership {events,summary,timeseries}` (`cli/commands/supabase-membership.ts`, filters `--org-slug` / `--outcome` / `--target-email`).
- **Dashboard UI** (the proxy is fully driveable from the dashboard, not just CLI/API): (1) `UpstreamTokensPage.tsx` scope select offers `supabase` (Bearer PAT) + `supabase_metrics` (HTTP Basic, with a username field defaulting to `service_role`); project refs validated as 20-char `[a-z0-9]` instead of 32-hex. (2) `PolicyBuilder.tsx` has two scope-gated action groups — `supabase` (Management API: `supabase:*` + per-category read/write across all `SupabaseCategory` values, including account-level `oauth` / `profile` / `snippets`) and `supabase_metrics` (`supabase:metrics:read`). `KeysPage.tsx` threads the new scope types through `makeDefaultPolicy` / `buildResourceHint` / `buildDefaultResources` (resources are `project:<ref>`) + the token dropdown groups. (3) `AnalyticsPage.tsx` surfaces `supabase_proxy_events` as a `supabase` source (mapper `fromSupabase` in `analytics-types.ts`, badge in `analytics-badges.tsx`, `getSupabaseProxyEvents` in `lib/api.ts`). NOTE: S3 has a SEPARATE policy surface (`S3PolicyBuilder` used by `S3CredentialsPage` — s3 keys use the S3-credentials model, not upstream tokens), so s3 actions deliberately do NOT appear in the main `PolicyBuilder`.
...
- **All three fronted upstreams are registered, each with the coverage model that fits its surface** — one uniform `CoverageProvider` interface, three different surface sources: `supabase` (live OpenAPI spec vs `classifySupabaseRequest`; 170 ops, all covered), `s3` (runtime enum `S3_OPERATIONS` from `src/s3/operations.ts` vs the real `detectOperation` routing, each op carrying a representative probe; 66 ops, all covered, completeness-guarded so the enum can't grow without a probe), `cloudflare` (live CF OpenAPI filtered to the proxied sub-resources, matched against the real Hono `app.routes` of each service sub-app; 132 in-surface ops, 116 covered, 16 deliberately allowlisted). The CF provider imports the service route sub-apps purely to read `.routes` — safe because nothing under `src/cf/` imports `cloudflare:workers` (only `durable-object.ts` does, via type-only edges). The discipline that keeps this honest: only filter CF ops *under prefixes we actually proxy* into the surface — never the whole CF API — so the allowlist stays small and meaningful instead of a 99% tautology.
