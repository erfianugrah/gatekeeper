...
The Supabase Management API proxy classifies each request to `supabase:<category>:<read|write>` across categories including `analytics`, `auth`, `billing`, `database`, `disk`, `domains`, `edge_functions`, `environment`, `networking`, `organizations`, `oauth`, `profile`, `projects`, `realtime`, `rest`, `secrets`, `snippets`, `storage`, and `metrics`. The `projects` category is a deliberate catch-all for project lifecycle (health, pause, restart, upgrade, restore, SSL, read-replicas, JIT, claim-token); finer-grained concerns are split out into their own categories (`billing` for compute/addon resize, `disk` for volume resize, `analytics` for usage telemetry, `networking` for IP bans/restrictions, `realtime` for the Realtime service config). Read vs write is derived from the HTTP method (with an explicit read-override set for the POST-but-read endpoints). Per-project metrics use the standalone `supabase:metrics:read` action. **Unclassified paths deny by default** (404) — the classifier is the RBAC surface, and an upstream that moves an endpoint fails safe (drift is surfaced by the api-coverage framework, not silently allowed).
