...
**`live-smoke` is a post-deploy gate** (`needs: deploy`, push-to-`main` only → always staging). It runs `cli/smoke-supabase.ts` (the focused Supabase entry — no CF creds) against the freshly deployed staging worker: a synthetic tier always runs, plus a real PAT-swap live tier when `SUPABASE_SMOKE_PAT` is set. Optional live metrics checks run when `SUPABASE_SMOKE_METRICS_SECRET` and `SUPABASE_SMOKE_METRICS_REF` are set. A non-destructive write-classified probe is enabled with `SUPABASE_SMOKE_ENABLE_WRITE_PROBE=1`. Missing optional vars only skip those subtiers — they do not fail the job. The job self-skips cleanly when `STAGING_ADMIN_KEY` is unset, so it never blocks a deploy before secrets are configured. Gated to push-to-`main` (never `workflow_dispatch`, which can target prod) because it mints real resources via the admin key. A separate weekly workflow (`supabase-live-smoke-scheduled.yml`) runs the same focused smoke against staging between deploys.
...
Smoke tests (`npm run smoke`) run against a live Gatekeeper instance and cover purge, S3 proxy, DNS proxy, CF proxy (`cf-proxy.ts`), the Supabase proxy (`supabase.ts` — also runnable in isolation via `npm run smoke:supabase` / `cli/smoke-supabase.ts`, which is what the CI `live-smoke` job uses; API-first checks run first against raw `/supabase/*` HTTP paths, with a synthetic tier that always runs plus an opt-in live tier gated on `SUPABASE_SMOKE_PAT` that registers a real PAT and exercises the real Supabase Management API through the proxy with a Gatekeeper key as Bearer. When the `supabase` binary is available, the suite also runs an official CLI compatibility check using the `sbp_`-shaped key path), analytics, bulk operations, config, and dashboard endpoints. Tests include granular policy enforcement: per-tag/host/prefix/URL-path scoping with wildcard/regex/set operators, cache-key header conditions, `url.query` param matching, cross-field AND conditions (host + path + header), Worker script-scoped keys, D1 `sql_command` conditions, KV `key_name` prefix conditions, S3 object-level/key-extension/key-prefix/filename conditions, multi-directory deny overlaps, and cross-service policies. All resources created during smoke tests (keys, S3 credentials, upstream tokens, upstream R2, DNS records, D1 databases, KV namespaces, config overrides, etc.) are tracked in `state` arrays and cleaned up in the orchestrator's `finally` block, even on crash.

For Supabase scope validation specifically, use this methodology:

- API-first assertions on proxied endpoints (`/supabase/v1/*`, supported `/supabase/v0/*`) are the source of truth.
- For each scope category, assert both allow and deny behavior (allow reaches upstream; deny returns `403`).
- Live scope tests should use a real upstream PAT (`SUPABASE_SMOKE_PAT`) so results reflect actual upstream-access bounds.
- `SUPABASE_SMOKE_ENABLE_WRITE_PROBE=1` enables a non-destructive write-classified probe (`select 1`).
- Metrics-secret live checks (`/supabase/metrics/:ref`) are optional until broad upstream availability.
...
| `npm run smoke:supabase` | Supabase-only smoke (API-first synthetic + opt-in live PAT/metrics tiers; no CF creds needed) |
