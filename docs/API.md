...
Fronts a stored Supabase credential (Personal Access Token or per-project metrics secret) with Gatekeeper's IAM + policy engine. Every inbound request is **classified** to a `supabase:<category>:<read|write>` action and authorized **before** the stored upstream credential is resolved. Unclassified paths **deny by default** (404). Categories include `analytics`, `auth`, `billing`, `database`, `disk`, `domains`, `edge_functions`, `environment`, `networking`, `organizations`, `oauth`, `profile`, `projects`, `realtime`, `rest`, `secrets`, `snippets`, `storage`, and `metrics`. The proxy also enforces an account-level rate limit (`supabase_rps` / `supabase_burst`, default 200/400) shared across both the Management and metrics surfaces — exhaustion returns `429` with a `Retry-After` header.
